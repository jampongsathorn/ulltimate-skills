...
| [`agentic-resourcefulness-mindset`](skills/agentic-resourcefulness-mindset/SKILL.md) | Think like a resourceful senior operator when the environment fights back — blocked egress, missing binaries, expired credentials, resets that delete work, unverifiable outputs. Forces Probe → Cheapest path → Escalate → Triangulate → Persist & hand off, with a capability-matrix script, an escalation ladder (up to CI relay), and a handoff template. | Original — for Claude Code + Arena |
