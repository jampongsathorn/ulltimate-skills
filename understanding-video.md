...
### STEP 3.7 — ถ้า push ไม่ผ่าน / ต้อง rebase (เจอจริงหลัง workspace ถูก re-clone)
    ...
    วิธีแก้:
      git log --oneline -3                                  # ดูว่า local ถอยไปที่ main หรือยัง
      git fetch origin '+refs/heads/<BRANCH>:refs/remotes/origin/<BRANCH>'
      # (clone ใหม่มักตั้ง remote.origin.fetch แค่ main → refspec ตรง ๆ แบบนี้จำเป็น)
      git rebase origin/<BRANCH>                            # replay งานใหม่ของเราทับของ remote
      # ⚠ หลัง rebase ไฟล์ workflow ชั่วคราวจะ "กลับมา" → git rm อีกครั้งก่อน push
